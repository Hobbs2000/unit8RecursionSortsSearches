public class StringReverse
{
    /**
     * Uses recursion to reverse a string
     */
    public static String reverse(String text)
    {
        if(text.length() == 0)
        {
            return text;
        }
        
        char c = text.charAt(0);
        String remaining = text.substring(1);
        String reverseText = reverse(remaining);
        reverseText += c;
        return reverseText;
    }
    
    /**
     * Reverses a string iteratively 
     */
    public static String reverseIter(String text)
    {
        String reverseText = "";
        
        for (int i = text.length()-1; i >= 0; i--)
        {
            char c = text.charAt(i);
            reverseText += c;
        }
        
        return reverseText;
    }
    
    public static void main(String[] args)
    {
        System.out.println("Recursive: "+StringReverse.reverse("Hello!"));
        System.out.println("Iterative: "+StringReverse.reverseIter("Hello!"));
        System.out.println("Expected: !olleH");
    }
}